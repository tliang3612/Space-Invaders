...
class Ship(Sprite):
    normal_images = [pg.image.load(f'images/Ship_{n}.png') for n in range(2)]
    exploding_images = [pg.image.load(
        f'images/explosion_0{n}.png') for n in range(8)]
    ...
    def __init__(self, game):
        ...
        self.firing = False
        self.frames = 0
        self.exploding_timer = Timer(
            image_list=Ship.exploding_images, delay=200, is_loop=False)
        self.normal_timer = Timer(
            image_list=Ship.normal_images, delay=1000, is_loop=True)
